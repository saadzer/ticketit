...
namespace Saadzer\Ticketit\Seeds;
...
class TicketitTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($this->statuses as $name => $color) {
            $status = \Saadzer\Ticketit\Models\Status::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        ...
        foreach ($this->categories as $name => $color) {
            $category = \Saadzer\Ticketit\Models\Category::create([
                'name'  => $name,
                'color' => $color,
            ]);
        ...
        foreach ($this->priorities as $name => $color) {
            $priority = \Saadzer\Ticketit\Models\Priority::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        $categories_qty = \Saadzer\Ticketit\Models\Category::count();
        $priorities_qty = \Saadzer\Ticketit\Models\Priority::count();
        $statuses_qty = \Saadzer\Ticketit\Models\Status::count();
        ...
        for ($u = 1; $u <= $this->users_qty; $u++) {
            ...
            for ($t = 1; $t <= $tickets_qty; $t++) {
                ...
                $category = \Saadzer\Ticketit\Models\Category::find($rand_category);
                ...
                $ticket = new \Saadzer\Ticketit\Models\Ticket();
                ...
                for ($c = 1; $c <= $comments_qty; $c++) {
                    ...
                    $comment = new \Saadzer\Ticketit\Models\Comment();
